...
void Lattice::DoIchidanDoushi(size_t index, const WStrings& fields, INT deltaCost)
{
    ...
    do {
        ...
        m_refs[index + node.pre.size()]++;
        // 「見て」
        if (str.size() >= 1 && str[0] == L'て') {
            node.pre += str[0];
            node.post += str[0];
            m_chunks[index].push_back(std::make_shared<LatticeNode>(node));
            m_refs[index + node.pre.size()]++;
            // 「見ていた」
            if (str.size() >= 3 && str[1] == L'い' && str[2] == L'た') {
                node.pre += L"いた";
                node.post += L"いた";
                node.katsuyou = SHUUSHI_KEI;
                m_chunks[index].push_back(std::make_shared<LatticeNode>(node));
                m_refs[index + node.pre.size()]++;
                // 「見ていたよ」「見ていたな」「見ていたね」
                if (str.size() >= 4 && (str[3] == L'よ' || str[3] == L'な' || str[3] == L'ね')) {
                    node.pre += str[3];
                    node.post += str[3];
                    node.katsuyou = SHUUSHI_KEI;
                    m_chunks[index].push_back(std::make_shared<LatticeNode>(node));
            ...
            if (str.size() >= 2 && str[1] == L'た') {
                ...
                m_refs[index + node.pre.size()]++;
                // 「見てたよ」「見てたな」「見てたね」
                if (str.size() >= 3 && (str[2] == L'よ' || str[2] == L'な' || str[2] == L'ね')) {
                    node.pre += str[2];
                    node.post += str[2];
                    node.katsuyou = SHUUSHI_KEI;
                    m_chunks[index].push_back(std::make_shared<LatticeNode>(node));
...
void Lattice::DoKahenDoushi(size_t index, const WStrings& fields, INT deltaCost)
{
    ...
    do {
        ...
        // 「来るよ」「来るね」「来るな」「来るぞ」
        if (str.size() < 1 ||
            (str[0] != L'よ' && str[0] != L'ね' && str[0] != L'な' && str[0] != L'ぞ'))
                break;
        node.pre += str[0];
        node.post += str[0];
        node.katsuyou = SHUUSHI_KEI;
