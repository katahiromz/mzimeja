...
// ê›íËÇ…âûÇ∂Çƒï∂éöóÒÇïœä∑Ç∑ÇÈÅB
...
std::wstring roman_to_hiragana(std::wstring roman)
{
    ...
    roman = lcmap(roman, LCMAP_HALFWIDTH); // éñëOÇ…îºäpÇ…ÇµÇƒÇ®Ç≠ÅB
...
// ãÂì_Ç‹ÇΩÇÕÉsÉäÉIÉhÇ©ÅH
// íçà”ÅFïœä∑ÇµÇ‚Ç∑Ç≥ÇÃÇΩÇﬂÅAÇ∏Ç⁄ÇÁÇ»îªíËÇ…ÇµÇƒÇ¢Ç‹Ç∑ÅB
BOOL is_period(WCHAR ch)
{
    return ch == L'ÅB' || ch == L'ÅD' || ch == L'.' || ch == L'°';
}

// ì«ì_Ç‹ÇΩÇÕÉRÉìÉ}Ç©ÅH
// íçà”ÅFïœä∑ÇµÇ‚Ç∑Ç≥ÇÃÇΩÇﬂÅAÇ∏Ç⁄ÇÁÇ»îªíËÇ…ÇµÇƒÇ¢Ç‹Ç∑ÅB
...
// ÉnÉCÉtÉìÇ‹ÇΩÇÕí∑âπÇ©ÅH
// íçà”ÅFïœä∑ÇµÇ‚Ç∑Ç≥ÇÃÇΩÇﬂÅAÇ∏Ç⁄ÇÁÇ»îªíËÇ…ÇµÇƒÇ¢Ç‹Ç∑ÅB
BOOL is_hyphen(WCHAR ch)
{
    return ch == L'-' || ch == L'Å|' || ch == L'Å[' || ch == L'∞';
}
