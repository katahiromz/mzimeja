...
#else
    if (table0[i] == (char)ch) return (i + 1);
#endif
  }
  return 0;
}
...
static WORD table5[] = {/* 0xa1 - 0xdf hiragana */
                        ...
                        0x82a1, 0x82a3, 0x82a5, 0x82a7, 0x82e1, 0x82e3, 0x82e5,
                        0x82c1, 0x815b, 0x82a0, 0x82a2, 0x82a4, 0x82a6, 0x82a8,
                        0x82a9, 0x82ab, 0x82ad, 0x82af, 0x82b1, 0x82b3, 0x82b5,
                        0x82b7, 0x82b9, 0x82bb, 0x82bd, 0x82bf, 0x82c2, 0x82c4,
                        0x82c6, 0x82c8, 0x82c9, 0x82ca, 0x82cb, 0x82cc, 0x82cd,
                        0x82d0, 0x82d3, 0x82d6, 0x82d9, 0x82dc, 0x82dd, 0x82de,
                        0x82df, 0x82e0, 0x82e2, 0x82e4, 0x82e6, 0x82e7, 0x82e8,
                        0x82e9, 0x82ea, 0x82eb, 0x82ed, 0x82f1, 0x814a, 0x814b};
static WORD table6[] = {/* 0xa1 - 0xdf katakana */
                        0x8142, 0x8175, 0x8176, 0x8141, 0x8145, 0x82f0, 0x8340,
                        0x8342, 0x8344, 0x8346, 0x8348, 0x8383, 0x8385, 0x8387,
                        0x8362, 0x815b, 0x8341, 0x8343, 0x8345, 0x8347, 0x8349,
                        0x834a, 0x834c, 0x834e, 0x8350, 0x8352, 0x8354, 0x8356,
                        0x8358, 0x835a, 0x835c, 0x835e, 0x8360, 0x8363, 0x8365,
                        0x8367, 0x8369, 0x836a, 0x836b, 0x836c, 0x836d, 0x836e,
                        0x8371, 0x8374, 0x8377, 0x837a, 0x837d, 0x837e, 0x8380,
                        0x8381, 0x8382, 0x8384, 0x8386, 0x8388, 0x8389, 0x838a,
                        0x838b, 0x838c, 0x838d, 0x838f, 0x8393, 0x814a, 0x814b};
static WORD table7[] = {/* sonant char part 1 */
                        0x82cd, 0x82d0, 0x82d3, 0x82d6, 0x82d9, 0x836e,
                        0x8371, 0x8374, 0x8377, 0x837a, 0x0000};
static WORD table8[] = {/* sonant char part 2 */
                        0x82a9, 0x82ab, 0x82ad, 0x82af, 0x82b1, 0x82b3, 0x82b5,
                        0x82b7, 0x82b9, 0x82bb, 0x82bd, 0x82bf, 0x82c2, 0x82c4,
                        0x82c6, 0x82cd, 0x82d0, 0x82d3, 0x82d6, 0x82d9, 0x834a,
                        0x834c, 0x834e, 0x8350, 0x8352, 0x8354, 0x8356, 0x8358,
                        0x835a, 0x835c, 0x835e, 0x8360, 0x8363, 0x8365, 0x8367,
...
#endif
{
  ...
  if (HIBYTE(code)) {
    ...
    if (code >= 0x30 && 0x39 >= code) return (code - 0x30 + 0x824f);
    if (code >= 0x41 && 0x5a >= code) return (code - 0x41 + 0x8260);
    if (code >= 0x61 && 0x7a >= code) return (code - 0x61 + 0x8281);
    if (code >= 0x20 && 0x40 >= code) return (table1[code - 0x20]);
    if (code >= 0x3a && 0x3f >= code) return (table2[code - 0x3a]);
    if (code >= 0x5b && 0x60 >= code) return (table3[code - 0x5b]);
    if (code >= 0x7b && 0x7e >= code) return (table4[code - 0x7b]);
...
WORD PASCAL ZenToHan(WORD code) {
...
#else
  if (code >= 0x824f && code <= 0x8258) return (code - 0x824f + 0x30);
  if (code >= 0x8260 && code <= 0x8279) return (code - 0x8260 + 0x41);
  if (code >= 0x8281 && code <= 0x829a) return (code - 0x8281 + 0x61);
  ...
  for (i = 0; i < 5; i++)
    if (code == table4[i]) return (0x7b + i);
  for (i = 0; i < 63; i++) {
    if (code == table5[i]) return (0xa1 + i);
    if (code == table6[i]) return (0xa1 + i);
  }
#endif
  return 0;
}
...
void PASCAL lHanToZen(LPMYSTR lpDst, LPMYSTR lpSrc, DWORD fdwConversion) {
  ...
  *lpDst = MYTEXT('\0');
  ...
  while (*lpSrc) {
    ...
    lpSrc++;
    if ((code1 == (0x00FF & '\xDE')) || (code1 == (0x00FF & '\xDF'))) {
